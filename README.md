...
This skeleton plugin is designed to:
...
This skeleton includes examples of:
...
```bash
go get github.com/nicolasbonnici/gorest-skeleton
```
...
See [DEVELOPMENT.md](DEVELOPMENT.md) for detailed instructions on forking and customizing this skeleton.

**Quick steps:**

1. Fork/clone this repository
2. Replace "skeleton" with your plugin name throughout
...
The plugin creates a `skeleton_items` table with:
...
```bash
GET /api/skeleton?limit=20&offset=0
Authorization: Bearer <token>
```
...
├── README.md              # This file
├── DEVELOPMENT.md         # Guide for creating plugins from this skeleton
├── LICENSE                # MIT License
...
├── migrations/
│   └── 001_create_skeleton_items.sql  # Database migration
├── examples/
│   └── basic/
...
This skeleton demonstrates the following architectural patterns used across GoREST plugins:
...
func (p *SkeletonPlugin) Name() string
func (p *SkeletonPlugin) Initialize(config map[string]interface{}) error
...
git clone https://github.com/nicolasbonnici/gorest-skeleton.git
cd gorest-skeleton
...
The skeleton plugin automatically uses user authentication from the auth plugin.
...
This skeleton is designed to be forked and customized. See [DEVELOPMENT.md](DEVELOPMENT.md) for a complete guide on:
...
Contributions to improve this skeleton are welcome! Please:
...
This skeleton can be adapted for various plugin types:
...
**Q: Can I use this skeleton for commercial projects?**
A: Yes! It's MIT licensed.

**Q: Do I need to keep the skeleton branding?**
...
Ensure the auth plugin is registered and configured properly. The skeleton plugin requires authenticated users for most operations.
...
- GitHub Issues: https://github.com/nicolasbonnici/gorest-skeleton/issues
...
- Initial skeleton plugin release
