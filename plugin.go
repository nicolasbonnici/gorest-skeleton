...
func (p *SkeletonPlugin) Initialize(config map[string]interface{}) error {
	...
	if err := p.config.Validate(); err != nil {
		logger.Log.Error("Invalid skeleton plugin configuration", "error", err)
		return err
	}
	...
	logger.Log.Info("Skeleton plugin initialized successfully", "enabled", p.config.Enabled)
	return nil
}
...
func (p *SkeletonPlugin) Config() *Config {
	return &p.config
}
