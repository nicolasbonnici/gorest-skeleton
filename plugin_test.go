...
func TestNewPlugin(t *testing.T) {
	...
	skeletonPlugin, ok := plugin.(*SkeletonPlugin)
	if !ok {
		t.Fatal("NewPlugin did not return *SkeletonPlugin")
	}

	if skeletonPlugin.Name() != "skeleton" {
		t.Errorf("Expected plugin name 'skeleton', got '%s'", skeletonPlugin.Name())
	}
}
