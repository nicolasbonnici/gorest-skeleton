...
This is the **gorest-skeleton** plugin, a comprehensive template/boilerplate for creating new GoREST plugins. It demonstrates all common patterns, structures, and best practices used across the GoREST plugin ecosystem.
...
The skeleton implements the core GoREST plugin interface in `plugin.go`:

```go
type Plugin interface {
    Name() string                                    // Returns "skeleton"
...
type Repository interface {
    Create(ctx context.Context, item *models.SkeletonItem) error
    GetByID(ctx context.Context, id string) (*models.SkeletonItem, error)
    List(ctx context.Context, limit, offset int) ([]models.SkeletonItem, int, error)
    Update(ctx context.Context, id string, item *models.SkeletonItem) error
...
```go
type CreateSkeletonItemRequest struct {
...
```sql
-- migrations/001_create_skeleton_items.sql
...
The skeleton registers these routes under `/api/skeleton`:

```
POST   /api/skeleton       - Create new item
GET    /api/skeleton/:id   - Get item by ID
GET    /api/skeleton       - List all items (paginated)
PUT    /api/skeleton/:id   - Update item
DELETE /api/skeleton/:id   - Delete item
```
...
```json
GET /api/skeleton?limit=10&offset=0
...
```go
logger.Log.Info("Creating skeleton item", "name", req.Name)
...
When creating a new plugin from this skeleton:

1. **Clone and rename**: `cp -r gorest-skeleton gorest-yourplugin`
2. **Global find/replace**: `skeleton` → `yourplugin`, `Skeleton` → `YourPlugin`
